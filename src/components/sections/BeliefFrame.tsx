...
export interface BeliefFrameBindings {
  ...
  chrome: ElementBinding<HTMLElement>;
}
...
export default function BeliefFrame({ bind }: { bind: BeliefFrameBindings }) {
  return (
    ...
    >
        ...
        <div className="mx-auto grid h-full w-full max-w-[1720px] grid-cols-12 items-end px-[var(--gutter-x)] pt-28 pb-24 lg:items-center lg:pb-16">
          <div className="col-span-12 max-w-[34rem] lg:col-span-5 lg:max-w-none lg:pr-4">
            <div className="mb-6 flex flex-wrap items-center gap-3">
              <p className="inline-flex items-center gap-2 rounded-full border border-[var(--electric-violet)]/30 bg-[#080D1F]/90 px-3 py-1">
                <span aria-hidden="true" className="h-2 w-2 rounded-full bg-[var(--neon-cyan)] shadow-[0_0_8px_var(--neon-cyan)]" />
                <span className="font-[family-name:var(--font-mono)] text-[10px] uppercase tracking-widest text-[var(--text-soft)]">
                  {BELIEF_COPY.label}
                </span>
              </p>
              <span className="hidden font-[family-name:var(--font-mono)] text-[10px] uppercase tracking-widest text-[var(--text-muted)] sm:inline">
                {BELIEF_COPY.subLabel}
              </span>
            </div>
      ...
      {/* Continuum bar (large screens) */}
      <div
        ref={(element) => bind.chrome(element)}
        className="invisible absolute inset-x-0 bottom-0 hidden items-center justify-between border-t border-white/[0.05] bg-[#03040A]/85 px-[var(--gutter-x)] py-3.5 font-[family-name:var(--font-mono)] text-[11px] tracking-wider text-[var(--text-muted)] opacity-0 lg:flex"
      >
        <span className="uppercase">{BELIEF_COPY.label}</span>
        <span className="flex items-center gap-2 uppercase">
          <span>Continuum:</span>
          <span className="font-medium text-[var(--text-primary)]">Section 05</span>
          <span className="text-white/20">/</span>
          <span>15</span>
          <span className="text-white/20">·</span>
          <a
            href="#journey"
            onClick={(event: MouseEvent<HTMLAnchorElement>) => navigateToHash(event, "#journey")}
            className="pointer-events-auto flex items-center gap-1 text-[var(--neon-cyan)] transition-colors hover:text-cyan-200"
          >
            Proceed to 06 — Our Journey <span aria-hidden="true">↓</span>
          </a>
        </span>
