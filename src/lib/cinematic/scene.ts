...
export const TEXT_WINDOWS: [number, number, number, number][] = [
  ...
  [0.24, 0.29, 0.37, 0.42], // 02 — The Gate of PAI
  [0.43, 0.48, 0.57, 0.62], // 03 — Enter
...
function computeCinematicFrame(p: number, rig: SceneRig): SceneFrame {
  ...
  return {
    ...
    // Frames 01–04 use the editorial scrim; Frame 05 has its own grade layer.
    scrim: Math.max(...text.slice(0, 4).map((t) => t.o)),
    text,
